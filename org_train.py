...
from concrete.agp_ipcae import IndirectConcreteClassifier as AdaptiveIndirectConcreteClassifier
...
class Trainer:
    ...
    def __init__(self, k_feature: int, input_dim: int, hidden_dim_cls: int, embedding_dim: int, meta_data: MetaData, device, epochs=200):
        ...
        self.model_dict = {
            ...
            'adaptive_indirect_concrete_cls': AdaptiveIndirectConcreteClassifier(
                input_dim=input_dim, hidden_dim_cls=hidden_dim_cls, output_dim=meta_data.cls_num, k_feature_select=k_feature, total_epochs=epochs, embedding_dim=embedding_dim, device=device
            ).to(device)
        }
        ...
        self.optimizers = {
            ...
            'adaptive_indirect_concrete_cls': torch.optim.Adam(self.model_dict['adaptive_indirect_concrete_cls'].parameters(), lr=1e-3)
    ...
    def _train_val_adaptive_ipcae_cls_epoch(self, epoch):
        model = self.model_dict['adaptive_indirect_concrete_cls']
        model.train()
        model.update_temperature(epoch)
        running_loss_train = 0.0
        
        for batch_idx, batch in enumerate(self.train_loader):
            self.optimizers['adaptive_indirect_concrete_cls'].zero_grad()
            y_pred = model(batch.X)
            loss = F.cross_entropy(y_pred, batch.obs['cell_type'])
            loss.backward()
            self.optimizers['adaptive_indirect_concrete_cls'].step()
            running_loss_train += loss.item()
        ...
        all_labels = np.array(all_labels)
        cls_report_frame = pd.DataFrame(classification_report(all_labels, all_preds, output_dict=True, zero_division='warn')).transpose()
        cls_report_frame.columns = [f'adaptiveipcaec_{col}' for col in cls_report_frame.columns]
        avg_test_loss = running_loss_test / len(self.test_loader.dataset)
        accuracy = correct / len(self.test_loader.dataset)
        
        # 根据测试集上的准确率决定是否进行网络结构调整，每10轮调整一次
        if epoch % 10 == 0:
            performance_metric = accuracy
            model.adapt(performance_metric, epoch)
            # 同时更新模型的优化器
            self.optimizers['adaptive_indirect_concrete_cls'] = torch.optim.Adam(model.parameters(), lr=1e-3)

        return avg_train_loss, avg_test_loss, accuracy, cls_report_frame, model.now_feature
    ...
    def train(self):
        ...
        for epoch in range(self.epochs):
            ...
            result_adaptive_ipcae = self._train_val_adaptive_ipcae_cls_epoch(epoch=epoch)
            ...
            self.result_dict['adaptive_indirect_concrete_cls']['train_loss'].append(result_adaptive_ipcae[0])
            self.result_dict['adaptive_indirect_concrete_cls']['test_loss'].append(result_adaptive_ipcae[1])
            self.result_dict['adaptive_indirect_concrete_cls']['test_acc'].append(result_adaptive_ipcae[2])
            
            report = result_adaptive_ipcae[3]
            report['k_feature'] = result_adaptive_ipcae[4]
            
            self.result_dict['adaptive_indirect_concrete_cls']['report'] = result_adaptive_ipcae[3]
            ...
            self.result_dict['adaptive_indirect_concrete_cls']['report'].to_csv(f'./result_cls_{self.train_name}/adaptive_indirect_concrete_cls_report.csv')
            ...
            logger.info(f"Epoch {epoch + 1}/{self.epochs}, Adaptive iPCAE k_feature: {result_adaptive_ipcae[4]}, Train Loss: {result_adaptive_ipcae[0]:.4f}, Adaptive iPCAEC Test Loss: {result_adaptive_ipcae[1]:.4f}, Adaptive iPCAEC Accuracy: {result_adaptive_ipcae[2]:.4f}")
